module ith::statement {
  ...
  public struct Statement has  store, copy, drop {
  ...
  public fun new_statement(statement_name : StatementName, allowed_values : VecSet<StatementValue>, allow_any : bool, condition : Option<StatementValueCondition>) : Statement {
    Statement {
      statement_name,
      allowed_values,
      condition,
  ...
  public(package) fun statements_match_many(self : &Statements, properties : &VecMap<StatementName, StatementValue>, current_time_ms : u64)  : bool {
      let statement_names = properties.keys() ;
      let mut  idx = 0;

      while (idx < statement_names.length())  {
        if (! self.statements_match(&statement_names[idx], properties.get(&statement_names[idx]), current_time_ms)) {
          return false
        };
        idx = idx +1;
      };
  ...
  public(package) fun statements_match(self : &Statements, statement_name : &StatementName, statement_value : &StatementValue, current_time_ms : u64) : bool {
    if ( ! self.data.contains(statement_name) ) {
      return false
    };
    self.data.get(statement_name).matches_name_value(statement_name, statement_value, current_time_ms)
  }
  ...
  public(package) fun matches_name_value(self: &Statement, name: &StatementName, value: &StatementValue, current_time_ms : u64) : bool {
  ...
  public(package) fun matches_value(self : &Statement, value : &StatementValue, current_time_ms : u64) : bool {
  ...
  public(package) fun to_map_of_statements(statements : vector<Statement>) : VecMap<StatementName, Statement> {
    let mut idx = 0;
