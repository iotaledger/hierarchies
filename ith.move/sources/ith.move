...
module ith::main {
  ...
  public fun revoke_statement(federation : &mut Federation, cap : &RootAuthorityCap, statement_name : StatementName, valid_to_ms : u64) {
  ...
  public fun validate_statements(self : &Federation, entity_id : &ID, statements: VecMap<StatementName, StatementValue>, ctx : &mut TxContext) {
    let current_time_ms = ctx.epoch_timestamp_ms();
  ...
  public fun accredit_to_attest(self : &mut Federation, cap : &AttestCap, receiver : ID, wanted_statements: vector<Statement>, ctx : &mut TxContext) {
    ...
    if (! self.is_root_authority(&ctx.sender().to_id())) {
      ...
      assert!(accreditations_to_accredit.are_statements_compliant(&wanted_statements, current_time_ms), EUnauthorizedInsufficientAccreditationToAccredit);
    };
  ...
  public fun revoke_accreditation_to_attest(self : &mut Federation, cap : &AttestCap, entity_id : &ID,  permission_id : &ID, ctx : &mut TxContext) {
    let current_time_ms = ctx.epoch_timestamp_ms();
    assert!(cap.federation_id == self.federation_id(), EUnauthorizedWrongFederation);
    ...
    if (! self.is_root_authority(&ctx.sender().to_id())) {
      ...
      assert!(remover_accreditations.are_statements_compliant(&statements, current_time_ms), EUnauthorizedInsufficientAccreditationToAccreditToAttest);
    };
  ...
  public fun accredit(self : &mut Federation, cap : &AccreditCap,  receiver : ID, want_statements : vector<Statement>,  ctx : &mut TxContext) {
      ...
      if (! self.is_root_authority(&ctx.sender().to_id())) {
        let accreditations_to_accredit = self.get_accreditations_to_accredit(&ctx.sender().to_id());
        assert!(accreditations_to_accredit.are_statements_compliant(&want_statements, current_time_ms), EUnauthorizedInsufficientAccreditationToAccredit);
      };
  ...
  public fun revoke_accreditation_to_accredit(self : &mut Federation, cap : &AccreditCap, entity_id : &ID,  permission_id : &ID, ctx : &mut TxContext) {
    assert!(cap.federation_id == self.federation_id(), EUnauthorizedWrongFederation);
    ...
    if (! self.is_root_authority(&ctx.sender().to_id())) {
      ...
      let current_time_ms   = ctx.epoch_timestamp_ms();
      let (_, statements) = (*accreditation_to_revoke.statements()).into_keys_values() ;
      assert!(remover_permissions.are_statements_compliant(&statements, current_time_ms), EUnauthorizedInsufficientAccreditationToAccredit);
    };
