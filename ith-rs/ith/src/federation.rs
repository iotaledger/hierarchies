...
pub(crate) mod ops {
    ...
    S: Signer<IotaKeySignature>,
  {
    ...
    let property_names =
      trusted_property::new_property_name(property_name, &mut ptb, client.ith_package_id())?;
    ...
    for property_value in allowed_values {
      let value = match property_value {
        TrustedPropertyValue::Text(text) => {
          trusted_property::new_property_value_string(text, &mut ptb, client.ith_package_id())?
        }
        TrustedPropertyValue::Number(number) => {
          trusted_property::new_property_value_number(number, &mut ptb, client.ith_package_id())?
        }
      };
