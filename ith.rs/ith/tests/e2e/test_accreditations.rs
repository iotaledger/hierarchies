...
#[tokio::test]
async fn test_move_error_wrong_federation() -> anyhow::Result<()> {
    let client = get_funded_test_client().await?;

    // Create first federation
    let federation1_id = client
        .create_new_federation()
        .build_and_execute(&client)
        .await?
        .output
    ...
    // Add statement to federation1
    let statement_name = StatementName::from("test.statement");
    let mut allowed_values = HashSet::new();
    allowed_values.insert(StatementValue::Text("value".to_string()));

    client
        .add_statement(
            *federation1_id.object_id(),
            statement_name.clone(),
            allowed_values.clone(),
            false,
        )
    ...
    // Try to create accreditation on federation1 but with wrong capability
    // This should trigger EUnauthorizedWrongFederation (error code 1)
    let statement = Statement::new(statement_name).with_allowed_values(allowed_values);
    let receiver_id = ObjectID::random();

    // Note: This test may need to be adjusted based on how capabilities work
    // We want to trigger the wrong federation error
    let result = client
        .create_accreditation_to_attest(*federation2_id.object_id(), receiver_id, vec![statement])
        .build_and_execute(&client)
        .await;

    println!("Wrong federation error result: {:?}", result);

    // This should fail, let's see what error we get
    assert!(result.is_err(), "Expected error for wrong federation");
    ...
    if let Err(e) = result {
        ...
        if error_string.contains("abort") || error_string.contains("code") {
            println!("Found abort/code in error: {}", error_string);
        }
    }
...
#[tokio::test]
async fn test_move_error_insufficient_permissions() -> anyhow::Result<()> {
    let client = get_funded_test_client().await?;

    // Create a secondary client/account that won't have permissions
    let secondary_client = get_funded_test_client().await?;

    // Create federation with primary client (gives them root authority)
    let federation_id = client
        .create_new_federation()
        .build_and_execute(&client)
        .await?
    ...
    // Add statement with primary client
    let statement_name = StatementName::from("restricted.statement");
    let mut allowed_values = HashSet::new();
    allowed_values.insert(StatementValue::Text("restricted".to_string()));
    ...
    // Try to create accreditation with secondary client (should lack permissions)
    // This should trigger EUnauthorizedInsufficientAccreditationToAttest (error code 3)
    let statement = Statement::new(statement_name).with_allowed_values(allowed_values);
    let receiver_id = ObjectID::random();

    let result = secondary_client
        .create_accreditation_to_attest(*federation_id.object_id(), receiver_id, vec![statement])
        .build_and_execute(&secondary_client)
        .await;

    println!("Insufficient permissions error result: {:?}", result);
...
#[tokio::test]
async fn test_move_error_accreditation_not_found() -> anyhow::Result<()> {
    let client = get_funded_test_client().await?;
    ...
    // Try to revoke a non-existent accreditation
    // This should trigger EAccreditationNotFound (error code 5)
    let fake_user_id = ObjectID::random();
    let fake_permission_id = ObjectID::random();

    let result = client
        .revoke_accreditation_to_attest(*federation_id.object_id(), fake_user_id, fake_permission_id)
        .build_and_execute(&client)
        .await;

    println!("Accreditation not found error result: {:?}", result);

    assert!(result.is_err(), "Expected error for non-existent accreditation");
...
#[tokio::test]
async fn test_move_error_invalid_statement_condition() -> anyhow::Result<()> {
    let client = get_funded_test_client().await?;
    ...
    // Try to add a statement with invalid condition (allow_any=true AND specific values)
    // This should trigger EInvalidStatementValueCondition (error code 4)
    let statement_name = StatementName::from("invalid.statement");
    let mut allowed_values = HashSet::new();
    allowed_values.insert(StatementValue::Text("value1".to_string()));
    allowed_values.insert(StatementValue::Text("value2".to_string()));
    ...
    let result = client
        .add_statement(
            *federation_id.object_id(),
            statement_name,
            allowed_values,
            true, // allow_any=true with specific values should be invalid
        )
        .build_and_execute(&client)
        .await;

    println!("Invalid statement condition error result: {:?}", result);

    assert!(result.is_err(), "Expected error for invalid statement condition");
