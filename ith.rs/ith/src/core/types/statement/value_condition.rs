...
use crate::{
    core::{
        operations::move_names,
        types::{
            new_statement_name, new_statement_value_number, new_statement_value_string, Statement, StatementValue,
        },
    },
    utils,
};
...
impl MoveType for StatementValueCondition {
    fn move_type(package: ObjectID) -> TypeTag {
        TypeTag::from_str(format!("{}::{}::StatementValueCondition", package, move_names::MODULE_CONDITION).as_str())
...
pub(crate) fn new_statement(
    ...
    for statement in statements {
            ...
            Some(expression) => {
                let (condition_arg, constructor_name) = match expression {
                    ...
                    StatementValueCondition::GreaterThan(value) => {
                        (ptb.pure(value)?, ident_str!("new_condition_greater_than").into())
                    }
                    StatementValueCondition::LowerThan(value) => {
                        (ptb.pure(value)?, ident_str!("new_condition_lower_than").into())
                    }
                };
        ...
        let statement = ptb.programmable_move_call(
            package_id,
            ident_str!(move_names::MODULE_STATEMENT).into(),
